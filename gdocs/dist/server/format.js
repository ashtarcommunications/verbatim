...
heading1Style[DocumentApp.Attribute.HORIZONTAL_ALIGNMENT] =
...
heading2Style[DocumentApp.Attribute.HORIZONTAL_ALIGNMENT] =
	DocumentApp.HorizontalAlignment.CENTER;
...
heading3Style[DocumentApp.Attribute.HORIZONTAL_ALIGNMENT] =
	DocumentApp.HorizontalAlignment.CENTER;
...
function condense() {
	...
	if (!selection) {
		DocumentApp.getUi().alert(
			'No text is selected.  Please select a text region in which line breaks are to be removed.',
		);
		return;
	}
	...
	for (var i = 0; i < len; i++) {
		...
		if (
			(nextType == DocumentApp.ElementType.PARAGRAPH &&
				nextElement.getHeading() == DocumentApp.ParagraphHeading.NORMAL) ||
	...
	elements.forEach((element) => {
		...
		switch (format) {
			...
			case 'highlight':
				...
				partial
					? e.setBackgroundColor(start, end, color)
